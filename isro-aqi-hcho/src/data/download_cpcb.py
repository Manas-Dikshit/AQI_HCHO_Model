...
def main() -> None:
    ...
    # Resolve dates via config_utils.resolve_date when available
    try:
        from src.utils.config_utils import resolve_date

        start = resolve_date(args.start, "date_range.default_start") or args.start
        end = resolve_date(args.end, "date_range.default_end") or args.end
    except Exception:
        start = args.start
        end = args.end

    process_cpcb_directory(args.input_dir, args.output, start, end)
