...
def main() -> None:
    ...
    parser.add_argument("--start", default=None, help="Start date YYYY-MM-DD (optional, resolved from config/.env if omitted)")
    parser.add_argument("--end", default=None, help="End date YYYY-MM-DD (optional, resolved from config/.env if omitted)")
    ...
    # Resolve dates via config/.env when possible (CLI -> .env -> config)
    start = args.start
    end = args.end
    ...
    # Dynamic fallback to recent 1-year window when unspecified
    ...
    if args.method == "gee":
        download_gee(args.pollutants, start, end, args.output_dir, args.gee_project)
    else:
        for poll in args.pollutants:
            download_dlr(poll, start, end, args.output_dir, args.dlr_user, args.dlr_pass)
