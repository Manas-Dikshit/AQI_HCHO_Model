...
def _build_synthetic_features(
    ...
    If `start`/`end` are omitted, they are resolved from `.env` or
    `config/paths.yaml` via :func:`src.utils.config_utils.resolve_date`.
    """
    # Resolve dates from CLI/.env/config when not explicitly provided
    try:
        from src.utils.config_utils import resolve_date

        if start is None:
            start = resolve_date(None, "date_range.default_start") or start
        if end is None:
            end = resolve_date(None, "date_range.default_end") or end
    except Exception:
        pass

    # Final dynamic fallback: use recent 1-year window if still unspecified
