...
def train_baseline_models(
    ...
    # Resolve train/test split defaults from config/.env if not provided
    if train_end is None or test_start is None:
        try:
            from src.utils.config_utils import resolve_date
            if train_end is None:
                train_end = resolve_date(None, "date_range.train_end")
            if test_start is None:
                test_start = resolve_date(None, "date_range.test_start")
        except Exception:
            pass

        # Dynamic sensible defaults: train = 1 year ending today-365d, test starts one day later
        if train_end is None:
            train_end = (pd.Timestamp.today() - pd.Timedelta(days=365)).strftime("%Y-%m-%d")
        if test_start is None:
            try:
                # prefer one day after train_end
...
def evaluate_baseline_models(
    ...
    # Resolve test_start default using config/.env or dynamic recent fallback
    if test_start is None:
        try:
            from src.utils.config_utils import resolve_date
            test_start = resolve_date(None, "date_range.test_start")
    ...
    if test_start is None:
        # Default to last 365 days as a sensible evaluation window
        test_start = (pd.Timestamp.today() - pd.Timedelta(days=365)).strftime("%Y-%m-%d")
