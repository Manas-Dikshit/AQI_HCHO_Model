...
def generate_demo_data() -> dict[str, pd.DataFrame]:
    ...
    # Use configured defaults where possible to keep demo coverage up-to-date
    from src.utils.config_utils import resolve_date
    demo_start = resolve_date(None, "date_range.default_start")
    demo_end = resolve_date(None, "date_range.default_end")
    if demo_start is None:
        demo_start = (pd.Timestamp.today() - pd.Timedelta(days=365)).strftime("%Y-%m-%d")
    if demo_end is None:
        demo_end = pd.Timestamp.today().strftime("%Y-%m-%d")

    grid = _build_synthetic_features(n_cells=200, start=demo_start, end=demo_end)
    ...
    for _, cell in sample_cells.iterrows():
        for d in pd.date_range(demo_start, demo_end, freq="D"):
    ...
    hcho = _build_synthetic_hcho_dataset(hcho_path, n_cells=300, start=demo_start, end=demo_end)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_ctrl1:
        layer = st.selectbox(
            "Layer",
            [
                "Interactive Main Map",
                "Predicted AQI (gridded)",
                "Predicted AQI (GeoJSON, coarse)",
                "Observed AQI (stations)",
                "INSAT AOD (gridded)",
            ],
        )
        if grid_df is not None:
            ...
            from src.utils.config_utils import resolve_date
            dr_start = resolve_date(None, "date_range.default_start")
            dr_end = resolve_date(None, "date_range.default_end")
            min_date = pd.Timestamp(dr_start).date() if dr_start else (pd.Timestamp.today() - pd.Timedelta(days=365)).date()
            max_date = pd.Timestamp(dr_end).date() if dr_end else pd.Timestamp.today().date()

        # Default to the most recent available date for a near real-time view
        selected_date = st.date_input(
            "Select date", min_value=min_date, max_value=max_date,
            value=max_date,
        )

        # Quick interactive map for per-point AQI and HCHO hotspots
        if layer == "Interactive Main Map":
            try:
                ...
                st.error(f"Interactive map requires folium + streamlit_folium installed: {exc}")
            else:
                day_df = grid_df[grid_df["date"] == pd.Timestamp(selected_date)].copy() if grid_df is not None else pd.DataFrame()
                if day_df.empty:
                    ...
                    m = folium.Map(location=[22.0, 82.0], zoom_start=5, tiles="CartoDB positron")
                    fg_aqi = folium.FeatureGroup(name="Predicted AQI (points)")
                    ...
                    if hcho_df is not None:
                        seas = hcho_df.copy()
                        if "season" not in seas.columns:
                            seas["season"] = seas["date"].dt.month.map(SEASON_MAP)
                        month = pd.Timestamp(selected_date).month
                        season_key = SEASON_MAP.get(month, "winter")
                        season_df = seas[seas["season"] == season_key]
                        if not season_df.empty:
                            agg = season_df.groupby(["lat", "lon"])["hcho_column"].mean().reset_index()
                            thr = float(agg["hcho_column"].quantile(0.90))
                            hotspots = agg[agg["hcho_column"] >= thr]
                            fg_h = folium.FeatureGroup(name="HCHO hotspots")
    ...
    with col_ctrl2:
        if layer == "Predicted AQI (gridded)" and grid_df is not None:
            ...
            if nc_p.exists():
                try:
                    ...
                    ds = xr.open_dataset(nc_p)
                    var = "pm25_pred" if "pm25_pred" in ds.data_vars else list(ds.data_vars)[0]
                    times = ds["time"].values
                    sel_time = np.datetime64(selected_date)
                    # pick nearest available time
                    idx = int(np.argmin(np.abs(times - sel_time)))
                    arr = ds[var].isel(time=idx).values
                    # handle NaNs
                    ...
                    # compute bounds from lat/lon coords
                    lon_coords = ds["lon"].values
                    lat_coords = ds["lat"].values
                    res = float(np.abs(lon_coords[1] - lon_coords[0])) if len(lon_coords) > 1 else 0.1
                    lat_min = float(lat_coords[0] - res / 2.0)
                    lon_min = float(lon_coords[0] - res / 2.0)
                    lat_max = float(lat_coords[-1] + res / 2.0)
                    lon_max = float(lon_coords[-1] + res / 2.0)
                ...
                if day_df.empty:
                    ...
                    # continuous scatter-based heatmap
                    fig = _aqi_scatter_fig(day_df, "aqi", f"Predicted AQI — {selected_date}", s=10)
                    st.pyplot(fig)
                    _fig_download_btn(fig, f"aqi_map_{selected_date}.png")
                    plt.close(fig)
        ...
        elif layer == "Predicted AQI (GeoJSON, coarse)":
            # attempt to load coarse GeoJSON exported by pipeline
            geojson_p = DATA_DIR / "predicted_aqi_grids" / "predicted_pm25_coarse.geojson"
            if geojson_p.exists():
                import folium
                from streamlit_folium import st_folium
