...
└── processed/            # Final datasets used by models & dashboard
    ...
    ├── predicted_aqi_grids/        # NetCDF / GeoJSON / GeoTIFF model outputs for dashboard
    │   ├── predicted_pm25.nc       # Full-grid predictions (time, lat, lon)
    │   ├── predicted_pm25_coarse.geojson # Coarse 0.25° GeoJSON (fast choropleth)
    │   └── geotiffs/               # Per-day single-band GeoTIFFs (optional)
...
- All gridded data uses a **0.1° regular lat/lon grid** over India
    (8–37.5 °N, 68–97.5 °E).  Cell centres and IDs follow the format
    `CELL_{lat:.2f}_{lon:.2f}`. The canonical grid resolution is 0.1° (default)
    and is produced by `src.data.grid_definition.get_india_grid()`.
  
    Prediction exports produced by the deep model are saved as an xarray
    `Dataset` serialized to NetCDF at `data/processed/predicted_aqi_grids/predicted_pm25.nc`
    with dimensions `(time, lat, lon)`. A downsampled GeoJSON (0.25°) and
    optional per-day GeoTIFFs are also written for fast dashboard rendering
